...
"""
Slight changes to allow non-personal but hopefully novel facts.
"""
...
class Tools:
    ...
    async def _emit_event(
        self, emitter: Optional[Any], description: str, status: str, done: bool = False
    ):
        ...
        if emitter:
            try:
                ...
                (
                    await emitter(event_data)
                    if asyncio.iscoroutinefunction(emitter)
                    else emitter(event_data)
                )
            except Exception as e:
                logger.warning(f"Failed to emit event: {e}")

    async def _error_response(
        self, emitter: Optional[Any], msg: str, payload: Optional[dict] = None
    ) -> dict:
    ...
    async def _get_user_memories(self, user_id: str) -> List[Any]:
        ...
        valid_memories = [
            ...
            if getattr(mem, "id") and getattr(mem, "content")
        ]
        return sorted(valid_memories, key=lambda m: m.id)

    async def _get_or_create_embeddings(
        self, user_id: str, memories: List[Any]
        ...
        cached_embeddings = {
            str(mem.id): self._embedding_cache.get((user_id, str(mem.id)))
            for mem in memories
        }
        uncached_mems = [
            mem for mem in memories if cached_embeddings.get(str(mem.id)) is None
        ]
        hit_count = len(memories) - len(uncached_mems)
        if hit_count > 0:
            logger.info(
                f"Cache hits for user {user_id}: {hit_count} out of {len(memories)}"
            )

        if uncached_mems:
            logger.info(
                f"Cache miss for {len(uncached_mems)} memories for user {user_id}. Computing now."
            )
            try:
                ...
                logger.error(
                    f"Error computing embeddings for user {user_id}: {e}. Some memories will be skipped."
                )

        embedding_dim = st_model.get_sentence_embedding_dimension()
        device = st_model.device
        final_embeddings = [
            cached_embeddings.get(
                str(mem.id), torch.zeros(embedding_dim, device=device)
            )
            for mem in memories
        ]
    ...
    async def _find_similar_memories(
        self, query: str, memories: List[Any], threshold: float, user_id: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = [
                {"score": score.item(), "memory": memories[i]}
    ...
    async def _process_memory_item(
        self, item_raw: str, user_id: str, existing_memories: List[Any]
    ) -> Dict[str, Any]:
        content = item_raw
        ...
        try:
            update_threshold, duplicate_threshold = self._calculate_dynamic_thresholds(
                content
            )
            results = await self._find_similar_memories(
                content, existing_memories, update_threshold, user_id
            )
            ...
            if not results:
                new_mem = await self._db_call(
                    Memories.insert_new_memory, user_id=user_id, content=content
                )
                return (
    ...
    async def add_memories(
        ...
        """
        Store ONLY critical, stable, and significant facts about. Be extremely selective—reject temporary, subjective, speculative, trite, or common knowledge, content. Split complex information into atomic facts.
        ...
        EXAMPLES (Store → Recall):
            "Usual Magneto-Optical discs tested to retain data after exposure to 95degC and 1inch neodynium magnet."
            → Store: MagnetoOptical disc data survived 1inch neodynium magnet under 95degC temperature.
            → Recall: MagnetoOptical disc data has been known to survive combined 95degC temperature and 1inch neodynium magnet.
        ...
        async with user_lock:
            try:
                ...
                for item in input_data:
                    action_result = await self._process_memory_item(
                        item, user_id, existing_memories
                    )
                    base_payload["actions"].append(action_result)
                    base_payload["summary"][action_result["operation"]] += 1
                    if action_result.get("operation") == "add" and action_result.get(
                        "new_memory"
                    ):
                ...
                updated_ids = [
                    a.get("updated_memory_id")
                    for a in base_payload["actions"]
                    if a.get("operation") == "update"
                ]
                if updated_ids:
                    logger.info(
                        f"Invalidating cache for {len(updated_ids)} updated memories for user {user_id}"
                    )
                ...
                summary_parts = [
                    f"{count} {op_labels[op]}"
                    for op, count in base_payload["summary"].items()
                    if count
                ]
                final_summary_message = f"✅ Memory processing complete: {', '.join(summary_parts) or 'No changes'}."

                await self._emit_event(
                    __event_emitter__, final_summary_message, "complete", True
                )
    ...
    def _calculate_dynamic_thresholds(self, query: str) -> tuple[float, float]:
        ...
        duplicate_threshold = min(
            0.99, update_threshold * self.valves.DUPLICATE_THRESHOLD_MULTIPLIER
        )
    ...
    async def retrieve_memories(
        ...
        TRIGGERS (ALWAYS use when user mentions):
            ...
            ✓ Preferences, relationships, past statements
            ✗ General info queries without context
        ...
        try:
            ...
            if not query or not query.strip():
                ...
                return await self._error_response(
                    __event_emitter__, error_payload["message"], error_payload
                )

            user_lock = await self._get_user_lock(user_id)
            async with user_lock:
                await self._emit_event(
                    __event_emitter__, f'🔍 Searching for: "{query}"', "in_progress"
                )
                ...
                relevant_results = await self._find_similar_memories(
                    search_query, user_memories, retrieve_threshold, user_id
                )
                ...
                if not relevant_results:
                    ...
                    return await self._error_response(
                        __event_emitter__, msg, error_payload
                    )

                sorted_matches = [res["memory"].content for res in relevant_results][
                    : self.valves.MAX_RELEVANT_CONTEXT_RESULTS
                ]
                result = {
                    "current_date": current_date_str,
                    "query": query,
            ...
            return await self._error_response(
                __event_emitter__, msg, {**error_payload, "message": msg}
            )
